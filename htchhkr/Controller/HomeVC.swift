...
class HomeVC: UIViewController, Alertable {
            ...
            if status == false {
                DataService.instance.REF_TRIPS.child(self.currentUserId!).observe(.value, with: { (tripSnapshot) in
                    if let tripDict = tripSnapshot.value as? [String : Any] {
                        ...
                        } else if tripDict[TRIP_IS_ACCEPTED] as? Bool == true {
                            self.removeOverlaysAndAnnotations(forDrivers: true, forPassengers: true)
    ...
    @IBAction func centerMapBtnWasPressed(_ sender: Any) {
        DataService.instance.REF_USERS.child(currentUserId!).observeSingleEvent(of: .value, with: { (snapshot) in
            if snapshot.hasChild(TRIP_COORDINATE) {
...
extension HomeVC: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        if let annotation = annotation as? DriverAnnotation {
            let identifier = ANNO_DRIVER_IDENTIFIER
            var view: MKAnnotationView
            view = MKAnnotationView(annotation: annotation, reuseIdentifier: identifier)
            view.image = UIImage(named: ANNO_DRIVER)
            return view
        } else if let annotation = annotation as? PassengerAnnotation {
            let identifier = ANNO_PASSENGER_IDENTIFIER
            var view: MKAnnotationView
            view = MKAnnotationView(annotation: annotation, reuseIdentifier: identifier)
            view.image = UIImage(named: ANNO_PASSENGER)
            return view
    ...
    func performSearch() {
        ...
        search.start { (response, error) in
            if let error = error {
                self.showAlert(withTitle: ERROR_MSG_TITLE_SEARCH, andMessage: error.localizedDescription)
            } else {
                if response?.mapItems.count == 0 {
                    self.showAlert(withTitle: ERROR_MSG_TITLE_NO_RESULTS, andMessage: ERROR_MSG_NO_MATCHES_FOUND)
                } else {
